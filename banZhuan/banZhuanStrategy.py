...
class BanZhuanStrategy(object):
    ...
    def sell(self, security, quantity, exchange="huobi"):  # quantity is a string value
        if exchange == "huobi":
            ...
            self.timeLog(
                "火币限价卖单已被执行，执行数量：%f，收到的现金：%.2f" % (executed_qty, executed_qty * float(order_info["price"])))
            return executed_qty
    ...
    def buy(self, security, cash_amount, exchange="huobi", sell_1_price=None):  # cash_amount is a string value
        if exchange == "huobi":
            ...
            if res[u"status"] == u'error':
                self.timeLog("下达火币市价買单（数量：%s）失败, 结果是: %s！" % (quantity, res[u'err-msg']),
                             level=logging.ERROR)
                return None
            order_id = res[u"data"]
            # 查询订单执行情况
            ...
            executed_qty = float(order_info["amount"])
            self.timeLog("火币市价买单已被执行，执行数量：%f，花费的现金：%.2f" % (executed_qty, float(order_info["price"])))
            self.dataLog()
            return executed_qty
        elif exchange == "okcoin":
