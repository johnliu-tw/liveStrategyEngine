...
class StatArbSignalGenerator(object):
    ...
    def getAccuntInfo(self, update):
        ...
        huobiAcct = huobiAcct['data']['list']
        for dataset in huobiAcct:
            if dataset['type'] == "trade":
                if dataset['currency'] == "usdt":
                    huobi_cny_cash = float(dataset['balance'])
                elif dataset['currency'] == "btc":
                    huobi_cny_btc = float(dataset['balance'])
                elif dataset['currency'] == "ltc":   
                    huobi_cny_ltc = float(dataset['balance'])
            elif dataset['type'] == "frozen":
                if dataset['currency'] == "usdt":
                    huobi_cny_cash_frozen = float(dataset['balance'])
                elif dataset['currency'] == "btc":
                    huobi_cny_btc_frozen = float(dataset['balance'])
                elif dataset['currency'] == "ltc":   
                    huobi_cny_ltc_frozen = float(dataset['balance'])            
    ...
    def sell_market(self, security, quantity, exchange="huobi", sell_1_price=None, buy_1_price=None , open_diff=0.002):  # quantity is a string value
        if exchange == "huobi":
            ...
            if res[u"status"] == u'error':
                self.timeLog("下达火币市价卖单（数量：%s）失败, 结果是: %s！" % (quantity, res[u'err-msg']),
                             level=logging.ERROR)
                return None
            order_id = res[u"data"]
            # 查询订单执行情况
            ...
            self.timeLog(
                "火币市价卖单已被执行，执行数量：%f，收到的现金：%.2f" % (executed_qty, float(order_info["field-cash-amount"])))
            return executed_qty
        elif exchange == "okcoin":
            ...
            self.timeLog("下达okcoin市价卖单（数量：%s）失败, 结果是：%s")
            ...
            if retry_time == self.okcoin_order_query_retry_maximum_times:
                ...
                self.timeLog("okcoin市价卖单已被执行，执行数量：%f，收到的现金：%.2f" % (
                    executed_qty, executed_qty * order_info["data"]["filledCurrency"]))
                return executed_qty
    ...
    def buy_market(self, security, cash_amount, exchange="huobi", sell_1_price=None, buy_1_price=None, open_diff=0.002):  # cash_amount is a string value
        if exchange == "huobi":
            ...
            tmp = float(cash_amount) * float(sell_1_price)
            tmp = helper.downRound(tmp, 4)
            cash_amount = str(tmp)
            ...
            if helper.componentExtract(res, u"status", "") != u'ok':
            ...
            # 查询订单执行情况
            order_info = self.HuobiService.getOrderInfo(order_id, market)
            self.timeLog("下达如下火币市价买单，金额：%s" % sell_1_price)
            result = float(cash_amount) / float(sell_1_price)

            self.timeLog("數量：%s" % result)

            retry_time = 0
            print(order_info)
            while retry_time < self.huobi_order_query_retry_maximum_times and order_info["data"]["state"] != "filled":
                self.timeLog("等待%f秒直至订单完成" % self.orderWaitingTime)
                time.sleep(self.orderWaitingTime)
                order_info = self.HuobiService.getOrderInfo(order_id, market)
                print(str(order_info))
            ...
            executed_qty = float(order_info["data"]["amount"])
            self.timeLog(
                "火币市价卖单已被执行，执行数量：%f，收到的现金：%.2f" % (executed_qty, float(order_info["data"]["field-cash-amount"])))
            return executed_qty


            if executed_qty > 0:
                self.timeLog("火币市价买单已被执行，执行数量：%f，花费的现金：%.2f" % (executed_qty, float(order_info["data"]["processed_amount"])))
                return executed_qty
            else:
            ...
            pair_id = 14 # Magic number for the API latency issue 
            price = (float(buy_1_price) + float(sell_1_price)) / 2
            self.timeLog("做完处理后的下单金额:%s" % sell_1_price)
        ...
        while True:
            ...
            if in_or_out == 0:
                ...
                if Qty < self.huobi_min_quantity or Qty < self.okcoin_min_quantity:
                    ...
                    # step1: 先处理卖
                    executed_qty = self.sell_market(self.coinMarketType, str(Qty), exchange="okcoin", sell_1_price=okcoin_sell_1_price, buy_1_price=okcoin_buy_1_price)
                    if executed_qty is not None:
                        # step2: 再执行买
