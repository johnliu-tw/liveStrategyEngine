...
def push_to_web(data):
    r = requests.post('http://127.0.0.1:3000/get_log', json = data)
...
class TimeLogger(object):
    ...
    def timeLog(self, content, level=logging.INFO, push_web=False):
        ...
        data = {'data': str(datetime.datetime.now())+ " " + content}
        if push_web:
            push_to_web(self.data_log)
            self.data_log=[]
        else:
